...
LevelState::LevelState(StateStack& stack, Context ctx) : State(stack), mCurrentPage(1), mMaxPage(2) {
    ...
    title->changeShape({420,72,600,40});
    title->changeSize(40);
    title->changeText("LEVEL SELECTION");
...
void LevelState::draw() {
    ...
    switch (mCurrentPage) {
        case 1: {
            Texture2D preview1 = Resource::mTexture.get(TextureIdentifier::PREVIEW1);
            DrawTexture(preview1, 245, 229);
            Texture2D preview2 = Resource::mTexture.get(TextureIdentifier::PREVIEW2);
            DrawTexture(preview2, 873, 229);
            Texture2D preview3 = Resource::mTexture.get(TextureIdentifier::PREVIEW3);
            DrawTexture(preview3, 245, 575);
